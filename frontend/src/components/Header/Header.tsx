				...
				{({ categories, loading, errorMessage }) => (
					<header className="bg-white h-20">
						...
						<nav className="flex flex-row justify-between items-center h-full">
							...
							{(categories?.length && (
								<ul
									className={`flex flex-col lg:flex-row items-center lg:justify-start space-y-4 lg:space-y-0 absolute 
									...
									{categories.map((category, index) => (
										<li key={index} className="px-4">
											<Link
												...
												className={`uppercase hover:font-semibold hover:text-[#5ECE7B] no-underline relative
